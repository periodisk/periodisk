...
Regenerate the complete example set from the repository root with:

```console
PYTHONPATH=src python scripts/render_examples.py
```

The test suite regenerates the deterministic SVG subset and compares it with
the committed files. PDF outputs are generated by the same command but are not
compared byte-for-byte because PDF metadata may vary between environments.
